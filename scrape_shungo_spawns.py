...
def scrape_shungo_spawns():
    ...
    for item in result_array:
        ...
        # Try to find name from form mappings
        ...
        # Generate image URL
        if pokemon_name.startswith('Pokemon #'):
            # Use PokeAPI for unknown Pokémon
    ...
    if missing_ids:
        print(f"\n⚠️ Could not find names for {len(missing_ids)} IDs: {sorted(missing_ids)[:20]}...")
