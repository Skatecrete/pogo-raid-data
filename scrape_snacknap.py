...
def scrape_snacknap_raids():
    ...
    # Look for headings and their following lists
    for element in soup.find_all(['h2', 'h3', 'ul']):
        ...
        # If this is a list and we're in a tier, extract Pokemon
            ...
            for item in element.find_all('li'):
                # Try to find the Pokemon name in the link
                link = item.find('a')
                if link:
                    text = link.get_text()
    ...
    response = requests.get(max_url, headers=headers)
    soup = BeautifulSoup(response.text, 'html.parser')
    ...
    for element in soup.find_all(['h2', 'h3', 'ul']):
        if element.name in ['h2', 'h3']:
            ...
            elif 'Dynamax 5-Star' in heading or 'Dynamax Tier 5' in heading:
                current_tier = 'dynamax_tier5'
                print("📋 Found Dynamax Tier 5 section")
